...
| [A locked device is silent to OKGETLABELS, though its source says it refuses](FINDING-locked-device-is-silent.md) | low | The `hidprint("Error device locked")` never reaches the wire; only the status broadcast does. Locked and unresponsive are indistinguishable, so the library infers it from the broadcasts. |
| [A failed PIN attempt cannot be reliably cleared from the host](FINDING-pin-buffer-cannot-be-cleared.md) | medium | The buffer only grows; the clear gesture is guarded on !isfade and appends before it resets, so it can make things worse. Measured: 8 appends for a 7-digit PIN. |
| [The custom-credential path copies 256 bytes into a 70-byte member](FINDING-custom-credential-copy.md) | low | A redundant copy in an "OnlyKey required change" block clobbers the adjacent user entity on every vendor tunnel request. Contained within the struct - checked, because the first reading was that it overran the credential array. |
